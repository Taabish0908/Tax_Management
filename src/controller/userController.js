...
const createAnotherAdmin = async (req, res) => {
    try {
        ...
        if (req.role != 'Admin') { 
            return res.status(403).send({ status: false, msg: "You are not Authorised to create an Admin" }) 
        }
        ...
        if (!(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,15}$/.test(password.trim()))) { 
            return res.status(400).send({ status: false, msg: "provide uppercase,lowercase,character & number" })
         }
...
const getAllTaxPayer = async (req, res) => {
    try {
        ...
        if (allTaxPayer.length == 0) {
            return res.status(404).send({ status: false, msg: "No tax payer Available." })
        }
