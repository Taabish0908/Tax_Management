...
const taxPay = async (req, res) => {
    try {
        ...
        if (!taxPayer) {
            return res.status(404).send({ status: false, message: "Tax Payer Doesn't Exist" });
        }
...
const editTaxPayer = async function (req, res) {
    try {
        ...
        if ((req.role == "Tax Payer")) {
            return res.status(403).send({ status: false, msg: "Only Accountant & Admin are allowed for this job" })
        }
        let data = req.body
